...
export function StabilityGauge({psi,level='insufficient_data'}) {
  const value=psi==null?0:Math.min(.4,psi);const angle=-90+(value/.4)*180;return <div className="stability-gauge"><svg viewBox="0 0 260 145" role="img" aria-label={`Population stability ${level.replaceAll('_',' ')}`}><path className="gauge-track" d="M 35 125 A 95 95 0 0 1 225 125"/><path className="gauge-stable" d="M 35 125 A 95 95 0 0 1 96 36"/><path className="gauge-moderate" d="M 96 36 A 95 95 0 0 1 169 36"/><path className="gauge-critical" d="M 169 36 A 95 95 0 0 1 225 125"/><g transform={`rotate(${angle} 130 125)`}><line className="gauge-needle" x1="130" y1="125" x2="130" y2="47"/><circle cx="130" cy="125" r="7"/></g><text className="gauge-number" x="130" y="112" textAnchor="middle">{psi==null?'—':psi.toFixed(3)}</text></svg><div><span>Stable &lt; 0.10</span><span>Moderate 0.10–0.25</span><span>Significant &gt; 0.25</span></div></div>;
}
