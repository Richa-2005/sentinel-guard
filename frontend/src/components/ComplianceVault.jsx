...
import { Check, Clipboard, Download, Expand, FileText, Link2, RefreshCw, Search, ShieldCheck, X } from 'lucide-react';
...
function DownloadButton({record}){const download=()=>{if(!record?.report_text)return;const identifier=String(record.transaction_id||record.card_id||record.id||'report').replace(/[^a-z0-9_-]+/gi,'-').replace(/^-+|-+$/g,'').slice(0,80)||'report';const documentText=[`# Sentinel Guard Compliance Memorandum`,``,`- Record: ${record.id}` ,`- Transaction: ${record.transaction_id||'Not recorded'}`,`- Card: ${record.card_id||'Not recorded'}`,`- Recorded: ${utc(record.timestamp)}`,`- Chain state: Append-only`,``,improve(record.report_text),``].join('\n');const url=URL.createObjectURL(new Blob([documentText],{type:'text/markdown;charset=utf-8'}));const link=document.createElement('a');link.href=url;link.download=`sentinel-guard-${identifier}.md`;document.body.appendChild(link);link.click();link.remove();URL.revokeObjectURL(url)};return <button className="button button--secondary" onClick={download} disabled={!record?.report_text}><Download size={14}/>Download report</button>}
